...
/**
 ...
 This configuration contains accessibility labels, since the
 package currently doesn't have, and perhaps should not have,
 localized strings. Instead, if you want to change the texts,
 you can override the standard configuration as such:

 ```swift
 var config = StorySlideshowConfiguration.standard
 config.nextAccessibilityLabel = L10n.next  // For instance, if the app has an `L10n` enum
 StorySlideshowConfiguration.standard = config
 ```
 */
public struct StorySlideshowConfiguration {
    ...
    /**
     ...
     - Parameters:
       - isAnimated: Whether or not the slideshow animates the slide transitions, by default `true`.
       - slideDuration: The duration of each slide, by default `5`.
       - timeTickIncrement: The duration increment per tick, by default `0.05`.
...
public extension StorySlideshowConfiguration {
    ...
    static var standard = StorySlideshowConfiguration()
}
